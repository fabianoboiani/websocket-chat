// to keep the session id
var sessionId = '';

// name of the client
var name = '';

// ip address
var ipAddress = '';

// socket connection url and port
var socket_url = '10.200.11.37';
var port = '80';

$(document).ready(function() {
	
	var popOverSettings = {
	    placement: 'right',
	    //container: 'ul',
	    html: true,
	    selector: '[rel="popover"]',
	    content: function () {
	    	console.log($(this).html());
	    	console.log($(this).closest('li').find('#popover-content').html());
	    	//$(this).append($(this).find('#popover-content').html());
	    	$(this).closest('li').find('#popover-content').show();

	    	//TODO
	        //return $(this).closest('li').find('#popover-content').html();
	    	return '';
	    }
	}

	$('body').popover(popOverSettings);

	$("#form_submit, #form_send_message").submit(function(e) {
		e.preventDefault();
		join();
	});
	var altura = $(window).height(); $("#messages").css("height",(altura-150)+"px");
	$(window).resize(function() { var altura = $(window).height(); $("#messages").css("height",(altura-150)+"px"); });
});

var webSocket;

/**
 * Connecting to socket
 */
function join() {

	// Checking person name
	if ($('#input_name').val().trim().length <= 0) {
		alert('Enter your name');
	} else {
		name = $('#input_name').val().trim();

		$('#prompt_name_container').fadeOut(1000, function() {
			// opening socket connection
			openSocket();
		});
	}

	return false;
}

/**
 * Will open the socket connection
 */
function openSocket() {
	// Ensures only one connection is open at a time
	if (webSocket !== undefined && webSocket.readyState !== WebSocket.CLOSED) {
		return;
	}

	// Create a new instance of the websocket
	webSocket = new WebSocket("ws://" + location.hostname + ":" + (location.port || '80') + location.pathname +"/chat?name=" + name);

	/**
	 * Binds functions to the listeners for the websocket.
	 */
	webSocket.onopen = function(event) {
		$('#message_container').fadeIn();
		$('#header h1').hide();
		$('.show-notifications').fadeIn();
		$('.show-online-users').fadeIn();

		if (event.data === undefined)
			return;

	};

	webSocket.onmessage = function(event) {

		// parsing the json data
		parseMessage(event.data);
	};

	webSocket.onclose = function(event) {

	};
}

/**
 * Sending the chat message to server
 */
function send() {
	var message = $('#input_message').val();

	if (message.trim().length > 0) {
		sendMessageToServer('message', message);
		$('#input_message').val('');
	} else {
		alert('Please enter message to send!');
	}
}

/**
 * Closing the socket connection
 */
function closeSocket() {
	webSocket.close();

	$('#message_container').fadeOut(600, function() {
		$('#prompt_name_container').fadeIn();
		// clearing the name and session id
		sessionId = '';
		name = '';

		// clear the ul li messages
		$('#messages').html('');
		$("#header h1").show();
		$('p.online_count').hide();
		$('.show-notifications').hide();
		$('.show-online-users').hide();
	});
}

/**
 * Parsing the json message. The type of message is identified by 'flag' node
 * value flag can be self, new, message, exit
 */
function parseMessage(message) {
	var jObj = $.parseJSON(message);

	// if the flag is 'self' message contains the session id
	if (jObj.flag == 'self') {

		sessionId = jObj.sessionId;

	} else if (jObj.flag == 'new') {
		// if the flag is 'new', a client joined the chat room
		var new_name = 'You';

		// number of people online
		var online_count = jObj.onlineCount;

		$('p.online_count').html('Hello, <span class="green">' 
				+ name + '</span>. <b>'
				+ online_count + '</b> people online right now')
				.fadeIn();

		if (jObj.sessionId != sessionId) {
			new_name = jObj.name;
		}

		var li = '<li class="new"><span class="name" style="color: '+jObj.color+';">' 
				+ new_name + '</span> '
				+ jObj.message + '</li>';
		appendChatMessage(li);

	} else if (jObj.flag == 'message') {
		// if the json flag is 'message', it means somebody sent the chat
		// message

		var from_name = 'You';

		if (jObj.sessionId != sessionId) {
			from_name = jObj.name;
		}

		var $messageId = new Date().getTime();
		var li = '<li id='+$messageId+' class="name" style="border-color: '+jObj.color+';">'
				+ '<span class="name" style="color: '+jObj.color+';" rel="popover">' + from_name + '</span> '
				+ '<span id="popover-content" style="display: none;">'
					+ jObj.message
					+ '<span class="timestamp">'+jObj.time+'</span>'
				+ '</span>'
				+ '</li>';

		// appending the chat message to list
		appendChatMessage(li, $messageId);

		// notification
		notifyMe(jObj.sessionId == sessionId, jObj);

	} else if (jObj.flag == 'exit') {
		// if the json flag is 'exit', it means somebody left the chat room
		var li = '<li class="exit"><span class="name red">' 
				+ jObj.name + '</span> ' 
				+ jObj.message + '</li>';

		var online_count = jObj.onlineCount;

		$('p.online_count').html(
				'Hello, <span class="green">' 
				+ name + '</span>. <b>'
				+ online_count + '</b> people online right now');

		appendChatMessage(li);
	}
	refreshChatUsers(jObj);
}

/**
 * Appending the chat message to list
 */
function appendChatMessage(li, $messageId) {
	$('#messages').append(li);

	if ($messageId) {
		$('#'+$messageId).find('span.name').click();
	}

	// scrolling the list to bottom so that new message will be visible
	$('#messages').scrollTop($('#messages').height()*$('#messages').height());
}

/**
 * Refresh chat usres list
 */
function refreshChatUsers(jObj) {
	if (jObj.users) {
		$('.modal-body').empty();

		jObj.users.forEach(function(entry, index) {
			var user = (index+1)+': <span style="color:' +entry['color']+ ';">' + entry['name'] + '</span>';
			$('.modal-body').append(user).append('</br>');
		});
	}
}

/**
 * Sending message to socket server message will be in json format
 */
function sendMessageToServer(flag, message) {
	var json = '{""}';

	// preparing json object
	var myObject = new Object();
	myObject.sessionId = sessionId;
	myObject.message = message;
	myObject.flag = flag;

	// converting json object to json string
	json = JSON.stringify(myObject);

	// sending message to server
	webSocket.send(json);
}

// NOTIFICATIONS
var isActive;

var notification;
var popupNotificationActive;

window.onfocus = function () { 
	isActive = true;
	closeNotification();
}; 
window.onblur = function () { 
	isActive = false; 
}; 

//request permission on page load
document.addEventListener('DOMContentLoaded', function() {
	if (Notification.permission !== "granted") {
		Notification.requestPermission();
	}
});

function notifyMe(yourSelf, jObj) {
	if (isActive || yourSelf) {
		return; //skip
	}
	
	if (!Notification) {
		alert('Desktop notifications not available in your browser. Try Chromium.');
		return;
	}
	
	if (Notification.permission !== "granted") {
		Notification.requestPermission();
	} else {
		showNotification();
		showMessage(jObj);
	}
}

function showNotification() {
	var showNotifications = $('#showNotifications').is(':checked');
	var showMessages = $('#showMessages').is(':checked');
	if (popupNotificationActive || !showNotifications || showMessages) {
		return; //skip
	}

	notification = new Notification(
			'WinkApp',
			{
				icon : 'images/notification.png',
				body : "Hey there! You've been notified!",
			});

	notification.onclick = function() {
		popupNotificationActive = false;
		window.parent.focus();
	};

	notification.onclose = function() {
		popupNotificationActive = false;
	};

	popupNotificationActive = true;
}

function showMessage(jObj) {
	var showMessages = $('#showMessages').is(':checked');
	if (!showMessages) {
		return; //skip
	}

	var messageListener = new Notification(
		'WinkApp',
		{
			icon : 'images/message.png',
			body : jObj.name+": "+jObj.message,
		});
	messageListener.onclick = function() {
		window.parent.focus();
	};

	//auto close
	setTimeout(function () {
		if (messageListener) {
			messageListener.close();
		}
	}, 3000);
}

function closeNotification() {
	popupNotificationActive = false;
	if (notification) {
		notification.close();
	}
}